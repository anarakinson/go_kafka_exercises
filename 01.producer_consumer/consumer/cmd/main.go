...
import (
	...
	"github.com/anarakinson/go_kafka_exercises/01.producer_consumer/consumer/internal/app/consumer"
...
func main() {
	...
	config.Consumer.Group.Rebalance.GroupStrategies = []sarama.BalanceStrategy{
		sarama.NewBalanceStrategyRoundRobin(), // стратегия балансировки
	}
	config.Consumer.Offsets.Initial = sarama.OffsetOldest // Читаем с самого начала

	cons := consumer.NewConsumer([]string{os.Getenv("KAFKA_BROKERS")}, config)
	...
	// работа консюмера
	go func() {
		err := cons.RunConsumer(ctx, []string{"user-events"}, "user-events-group", os.Getenv("LOG_FILE"))
		if err != nil && !errors.Is(err, consumer.ErrCanceled) {
			logger.Log.Error(
				"Consumer failed",
				zap.Error(err),
			)
